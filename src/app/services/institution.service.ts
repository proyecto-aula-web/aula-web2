...
export class InstitutionService {
  ...
  constructor(private afs: AngularFirestore) {
    this.institutionCollection = this.afs.collection(
      'institutions',
      res => res
    );
  }
  ...
  addNewInstitution(institution: InstitutionInterface) {
    ...
    return userRef.set(institution, { merge: true });
  }
  ...
  getOneInstitution(idinstitution: string) {
    this.institutionDoc = this.afs.doc<InstitutionInterface>(
      `institutions/${idinstitution}`
    );
    this.institution = this.institutionDoc.snapshotChanges().pipe(
      map(action => {
